...
따라서 순서가 바뀐다. 0단계는 "깨지는지 확인"이 아니라 **소유권 키를 연결 단위로 고치는 일**이고,
...
분리해도 이 왕복은 **저절로 사라지지 않는다.** `WireviewMeta.send`(`core/meta.py:457`)는 자기
세션에도 `Broker.send_to_session`으로 보내고, 메서드가 어느 클래스에 있든 그 호출은 그대로다.
...
아니다 — 프로세스 기본 RSS, 큰 컴포넌트, 운영 레이어, 버퍼는 포함되지 않았다. 실제 배포 구성의
목표 연결 수에서 다시 재야 한다.
...
> 이 절의 수정은 Codex(gpt-6-astra) 리뷰(2026-09-09)의 지적을 반영한 것이다. 원래는 "다중 프로세스
