...
> **진행 상황.** 소유권·수명 갈래는 #77에서 고쳤다. 인덱스 키가 `(connection_id, component_id)`가
> 되고, 업로드 URL·토큰·진행 통지 그룹이 모두 연결 id를 달고, `disconnect()`가 그 연결의
> 레지스트리를 전부 해제한다. 분산 접근 갈래는 #83으로 남았고, 그때까지 청크는 WebSocket을
> 쥔 프로세스에 도달해야 한다(`docs/DEPLOYMENT.md`).
