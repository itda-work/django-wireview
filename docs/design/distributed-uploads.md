...
> 상태: **구현됨(A안). 2026-09-09.** 사용자 문서는 `docs/features/chunked-uploads.md`,
> 배포 조건은 `docs/DEPLOYMENT.md`. 이 메모는 왜 이 설계인지의 기록으로 남긴다.
> #77(소유권·수명 갈래)이 닫힌 뒤 남은 **분산 접근** 갈래였다.
...
설계와 다르게 간 곳과, 구현 중에야 드러난 것.

- **토큰을 문자열이 아니라 객체로 서명한다.** 5-1은 `wv2:{conn}:{comp}:{config}:{ref}:{max_bytes}`를
  적었지만 컴포넌트 id와 업로드 이름은 앱 템플릿에서 오므로 콜론이 들어갈 수 있다. 구분자 파싱이
  어긋나는 자리가 하필 "누가 무엇을 쓸 수 있는가"를 정하는 자리라 `sign_object`로 바꿨다.
  구형 4토막 토큰 수용은 그대로다. 확장자(`ext`)도 토큰에 넣었다 — 무상태 핸들러에는
  `entry.client_name`이 없는데 magic bytes 검사에 확장자가 필요하다.
- **disconnect도 마커를 남긴다.** 5-4는 연결 디렉터리를 통째로 지운다고 적었지만, 그러면 다른
  워커에서 진행 중이던 청크가 디렉터리를 다시 만들어 `.part`를 남긴다. 디렉터리는 두고 `.gone`을
  남기며 안의 파일만 지운다. 마커는 sweep이 걷는다.
- **`ATOMIC_REQUESTS = True`면 청크가 전부 500이었다.** `UploadView.post`가 async라 Django 핸들러가
  뷰를 부르기도 전에 `RuntimeError`를 낸다. #83과 무관한 기존 버그이고, 기존 테스트가 뷰를 직접
  호출해 핸들러를 건너뛰었기 때문에 **워커 둘짜리 E2E를 붙이고 나서야** 드러났다. URLconf에서
  모든 alias에 `non_atomic_requests`로 등록해 고쳤다.
- **크기 검증이 아예 없었다.** 5-1의 괄호가 지적한 그대로다. 토큰의 `max_bytes`로 막고 413을 준다.
- **완료 콜백은 WS 명령 쪽에서만 부른다.** 브로커의 `upload.completed`와 클라이언트의
  `upload_complete`가 둘 다 완료를 주장하므로, 승격은 파일을 stat해서 하고 `on_upload_complete`는
  한쪽에서만 부른다. 양쪽에서 부르면 두 번 실행된다.
- **AC4는 두 겹으로 붙었다.** `tests/test_distributed_uploads.py`가 독립 인터프리터로 청크를 보내
  #83을 재현했던 모양 그대로 검증하고, `tests/test_multiworker_uploads.py`(e2e)가 uvicorn 워커 둘 +
  브로커에서 WS는 A, 청크는 B로 보내 소유 워커가 그 바이트를 읽는 것까지 확인한다.
  픽스처 컴포넌트는 `tests/testproj/uploadprobe/`.
