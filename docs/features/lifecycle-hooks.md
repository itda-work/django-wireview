...
동기 템플릿 패스 안에서 async 훅을 부르려면 다리를 건너야 하고, 그것이 이 경로의 비용 전부다.
실측(Apple Silicon, 워밍업 후 중앙값):
...
`_on_mount`가 비어 있고 페이지에 경계도 없으면 다리를 아예 건너지 않는다 — 경계를 쓰지 않는
프로젝트는 이 비용을 한 푼도 내지 않는다.
