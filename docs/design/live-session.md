...
>
> **3라운드 리뷰(Codex gpt-6-astra, 2026-09-09)로 여덟 곳을 더 고쳤다.** 이 문서의 §3~§6은 봉투
> 결합에만 집중했는데, 실제 구멍은 그 옆에 있었다 — 마운트 훅이 **예외**를 던지는 경로, 중첩된 일반
> `{% component %}`, 첫 join을 미뤄 로그아웃 발행을 피하는 창, 세션 키를 인증 세대로 쓴 것(signed-cookie
> 백엔드에서 `session_key`는 쿠키 문자열 전체다), `STATE_ACCEPT_LEGACY`와 경계의 조합, async 뷰
> 데코레이터, 그리고 클라이언트가 경계 검증 **전에** 보내던 `params_changed`. 결론과 각 항목은
> [live-session-review-2026-09-09-round3.md](./live-session-review-2026-09-09-round3.md)에 있다.
