...
그것을 인라인으로 그리기 **직전**에 훅이 돌고, halt하면 그 자리에 아무것도 그려지지 않는다.

그 전에는 이 경로가 라이브 렌더에서 훅을 건너뛰었고, 이유는 "join이 이미 페이지를 덮었다"였다.
join이 덮은 것은 *페이지*였다 — 특정 자식 하나를 거절하려던 세션 훅은 돌지 않았고, 그 컴포넌트는
마크업뿐 아니라 이벤트 대상까지 되었다.

### 비용

동기 템플릿 패스 안에서 async 훅을 부르려면 다리를 건너야 한다. 아래는 **훅이 아무 일도 하지 않을
때** 그 다리 자체의 값이다 — 훅이 DB나 외부를 건드리면 그 비용이 따로 붙는다.

실측 조건: Apple Silicon, 워밍업 50회 뒤 200회 × 7세트의 중앙값, 시간만(할당 추적은 5배쯤 부풀린다).
재현기는 이 표의 근거일 뿐 저장소의 벤치 스위트가 아니다.
...
**인스턴스당 한 번**이라는 것이 핵심이다. 부모는 마운트보다 훨씬 자주 재렌더하므로, "이미 마운트됐나"를
...
5. 훅 안의 예외는 **halt와 같은 거절**이다. 예외 자체는 경로에 따라 올라가거나(WebSocket join이
   중단되고 클라이언트는 `remove`를 받는다, HTTP 렌더는 500) 로그에 남지만(LiveComponent 자식),
   어느 쪽이든 컴포넌트는 렌더되지 않고 저장소에도 남지 않는다. 인가 조회가 DB 오류로 실패하는 것이
   거절보다 통과하기 쉬워서는 안 되기 때문이다
