...
이미 공개로 돌던 페이지에 `@session.view`를 붙이는 전환에는 순서가 있다. 서버는 토큰이 말하는 경계만
보고 발급 시점은 모르므로, **그 전에 발급된 경계 없는 토큰은 새 정책을 지나지 않는다.**

1. **보호할 컴포넌트에 `_live_sessions`를 먼저 선언한다.** 이것만이 옛 토큰을 거절한다.
   `manage.py check`의 `wireview.W010`이 선언을 빠뜨린 컴포넌트를 알려 준다.
2. **`STATE_ACCEPT_LEGACY`는 끈다.** 경계가 선언되면 자동으로 무력화되지만, 켜 둔 채로 두면 W010이
   경고한다.
3. 옛 토큰을 즉시 끊어야 한다면 **`WIREVIEW["SIGNING_KEY"]`를 교체한다.** 열려 있던 페이지는 한 번
   reload한다. 이때 `SIGNING_KEY_FALLBACKS`에 옛 키를 남기면 옛 토큰이 계속 통과하므로, 이 목적의
   교체에서는 fallback을 두지 않는다.
4. 아무것도 하지 않으면 옛 토큰은 `STATE_MAX_AGE`(기본 14일)만큼 유효하다.
