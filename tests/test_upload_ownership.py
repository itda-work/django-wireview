"""Upload artefacts belong to a connection, not to a component id (#77).
...
needs on the server -- the registry index, the signed token, the HTTP endpoint and
the progress group -- is therefore scoped by the connection that owns it.
"""
...
from wireview.views import UploadView, get_upload_registry
...
async def join_uploader(consumer: WireviewConsumer, component_id: str) -> Component:
    ...
    await consumer._register_upload_registry(component)
...
async def test_two_connections_on_the_same_component_id_keep_their_own_registries():
    ...
    registry_a = get_upload_registry(consumer_a.connection_id, "bookmarks")
    registry_b = get_upload_registry(consumer_b.connection_id, "bookmarks")
    assert registry_a is not None and registry_b is not None
    assert registry_a is not registry_b


async def test_a_leave_on_one_connection_leaves_the_others_registry_and_file_alone():
    consumer_a, _ = await connect_consumer()
    consumer_b, _ = await connect_consumer()
    await join_uploader(consumer_a, "bookmarks")
    await join_uploader(consumer_b, "bookmarks")
    ...
    assert get_upload_registry(consumer_a.connection_id, "bookmarks") is None
    assert not file_a.exists()
    assert get_upload_registry(consumer_b.connection_id, "bookmarks") is registry_b
    assert file_b.exists()

    await consumer_b.disconnect(1000)


async def test_disconnect_releases_only_that_connections_registries_and_files():
    consumer_a, _ = await connect_consumer()
    consumer_b, _ = await connect_consumer()
    await join_uploader(consumer_a, "bookmarks")
    ...
    assert registry_a is not None and registry_b is not None
    view = UploadView()
    await post_chunk(view, consumer_a.connection_id, "bookmarks", add_entry(registry_a))
    ...
    assert get_upload_registry(consumer_a.connection_id, "bookmarks") is None
    assert get_upload_registry(consumer_a.connection_id, "gallery") is None
    assert not file_a.exists()
    assert get_upload_registry(consumer_b.connection_id, "bookmarks") is registry_b
    assert file_b.exists()
...
async def test_a_token_issued_on_one_connection_is_refused_on_the_others_url():
    ...
    assert registry_b.get_entry("images", "upload-1").temp_path is None
...
async def test_a_live_component_child_with_uploads_is_registered_after_the_parents_render():
    consumer, _ = await connect_consumer()

    board = await consumer.repo.join("OwnBoard", {"id": "board"})
    await consumer._register_upload_registry(board)
    await consumer.send_render(board)

    registry = get_upload_registry(consumer.connection_id, "child-1")
    ...
    assert get_upload_registry(consumer.connection_id, "child-1") is None
...
async def test_a_chunk_that_finishes_after_a_cancel_leaves_no_temp_file():
    ...
    registry = get_upload_registry(consumer.connection_id, "bookmarks")
...
async def test_the_progress_group_is_per_connection_and_subscribed_once():
    ...
    assert group not in outbound.subscribed, "nothing joins the group until a registry needs it"
