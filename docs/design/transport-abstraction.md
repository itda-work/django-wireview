...
같은 벤치로 Python 서버끼리도 비교했다. daphne는 5,000 연결에서도 연결당 45.3 KB(항목 5개)로 2,000 연결과 같아 선형이고, uvicorn은 `websockets` 구현으로 212 KB, `wsproto`로 200 KB를 써서 daphne의 4배가 넘는다(원인은 조사하지 않았다). Channels 워크로드에서는 daphne가 메모리 기준으로 가장 나은 Python 서버다.
